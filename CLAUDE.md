...
- **Saved data outlives the build that wrote it.** `seedIfEmpty()` only fills empty tables; `topUpSeedContent()` ([`lib/data/seed.ts`](lib/data/seed.ts)) runs on every load and adds what has shipped since — new missions, campaigns and cards — and refreshes the *content* of shipped cards without touching study state or the user's own cards. The mission operations only update existing rows, so a mission with no row can be played but never completed: that is what the top-up prevents. Adding content needs no version bump. Changing the *shape* of a record still needs a migration.
