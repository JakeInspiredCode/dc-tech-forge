...
- Full-height screens size themselves with `h-[calc(100vh-var(--chrome-h))]`, not a hard-coded nav height. `--chrome-h` grows when the sample-data banner is showing. Anything in that chrome must be *exactly* as tall as the variable says — a 1px border on the nav once gave every hub page a scrollbar.
